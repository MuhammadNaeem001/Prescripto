...
const Appointment = () => {
  ...
  const [selectedTimeIndex, setSelectedTimeIndex] = useState(null); // selected time index
      ...
      <div className='sm:ml-72 sm:pl-4 font-medium text-gray-700 mt-6'>
        ...
        <div className='flex gap-3 items-center w-full overflow-x-scroll mt-4'>
          {docSlots.length > 0 && docSlots.map((item, index) => (
            <div
              onClick={() => { setSlotIndex(index); setSelectedTimeIndex(null); }} // reset time selection when day changes
          ...
          {docSlots.length > 0 && docSlots[slotIndex] && docSlots[slotIndex].map((item, index) => (
            <p
              className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${
                selectedTimeIndex === index ? 'bg-[#5F6FFF] text-white' : 'border border-gray-200 text-gray-400'
              }`}
              key={index}
